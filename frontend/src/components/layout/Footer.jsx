...
const Footer = () => {
  ...
  const submit = (e) => {
    ...
    toast.success(t("newsletter_title"));
    setEmail("");
  };
  return (
    <footer className="eggi-dark text-white pt-20 pb-10">
      <div className="eggi-container">
        ...
        >
          ...
          <div>
            <h3 className="display-title text-3xl sm:text-4xl mb-2">{t("newsletter_title")}</h3>
            <p className="text-[#d9d3c6] font-medium mb-5">{t("newsletter_text")}</p>
            <form onSubmit={submit} className="flex flex-col sm:flex-row gap-3">
              ...
              <button type="submit" className="btn-brutal eggi-yellow text-[#1C1A17] font-bold px-6 py-3 rounded-xl hard-border" style={{ boxShadow: "3px 3px 0 0 #F76D5E" }}>{t("notify")}</button>
            </form>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-4 gap-8 border-t-2 border-[#3a352d] pt-10">
          <div>
            ...
            <p className="text-sm text-[#a49d90] font-medium max-w-xs">{t("footer_tagline")}</p>
          </div>
          <div>
            <h4 className="font-extrabold text-lg mb-2">{t("footer_shop")}</h4>
            <ul className="space-y-1 text-sm font-semibold text-[#d9d3c6]">
              <li><Link to="/shop" className="hover:text-yellow-eggi">{t("all_products")}</Link></li>
          ...
          <div>
            <h4 className="font-extrabold text-lg mb-2">{t("footer_account")}</h4>
            <ul className="space-y-1 text-sm font-semibold text-[#d9d3c6]">
              <li><Link to="/orders" className="hover:text-yellow-eggi">{t("my_orders")}</Link></li>
              <li><Link to="/login" className="hover:text-yellow-eggi">{t("home") && "Login"}</Link></li>
            </ul>
          </div>
          <div>
            <h4 className="font-extrabold text-lg mb-2">{t("footer_contact")}</h4>
            <a href={`mailto:${SITE.email}`} className="text-sm text-yellow-eggi font-semibold underline">{SITE.email}</a>
          </div>
        </div>
        <p className="text-xs text-[#6b6558] font-medium mt-10">© 2026 {SITE.trademark}. {t("rights")}</p>
      </div>
    </footer>
  );
};

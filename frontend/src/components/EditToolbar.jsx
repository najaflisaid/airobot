...
const EditToolbar = () => {
  const { editMode, setEditMode, save, dirty, saving, lang } = useContent();
  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[70] flex items-center gap-3 bg-white hard-border rounded-2xl px-4 py-3" style={{ boxShadow: "4px 4px 0 0 #1C1A17" }}>
      <span className="flex items-center gap-2 text-sm font-extrabold"><Pencil size={16} /> Edit mode · {String(lang || "").toUpperCase()}{dirty ? " · unsaved" : ""}</span>
...
export const EditModeBanner = () => {
  const { editMode, lang } = useContent();
  const { isAdmin } = useAuth();
  if (!isAdmin || !editMode) return null;
  return (
    <div className="fixed top-0 left-0 right-0 z-[65] bg-[#F76D5E] text-white text-center text-xs font-extrabold py-1.5 flex items-center justify-center gap-2">
      <Eye size={13} /> LIVE EDITOR ({String(lang || "").toUpperCase()}) – click any text on the page to edit it
    </div>
  );
};

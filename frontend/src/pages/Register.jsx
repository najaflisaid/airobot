...
const Register = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success("Account created! Welcome to NUVII.");
      navigate("/");
    } catch (err) {
  ...
  const google = async () => {
    try {
      ...
      toast.success("Welcome to NUVII!");
      navigate("/");
    } catch (err) {
      toast.error(mapError(err.code));
    }
  ...
  return (
    <div className="eggi-cream min-h-screen flex items-center justify-center px-4 pt-28 pb-16">
      <div className="w-full max-w-md bg-white hard-border rounded-3xl p-8" style={{ boxShadow: "6px 6px 0 0 #1C1A17" }}>
        ...
        <h1 className="display-title text-3xl mb-1">{t("create_account")}</h1>
        <p className="text-[#7a7266] font-medium mb-6">{t("register_sub")}</p>
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="text-sm font-bold">{t("name")}</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} className="w-full mt-1 px-4 py-3 rounded-xl hard-border outline-none font-medium" placeholder="Your name" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full mt-1 px-4 py-3 rounded-xl hard-border outline-none font-medium" placeholder="At least 6 characters" />
          </div>
        ...
        <button onClick={google} type="button" className="btn-brutal w-full bg-white font-bold px-4 py-3 rounded-xl hard-border flex items-center justify-center gap-2" style={{ boxShadow: "3px 3px 0 0 #B9E0F5" }}>
        ...
        <p className="text-sm font-medium text-center mt-6">
          {t("have_account")} <Link to="/login" className="font-bold underline">{t("login_btn")}</Link>
        </p>
      </div>
    </div>
  );

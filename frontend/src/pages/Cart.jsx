    ...
    return (
      <div className="eggi-cream min-h-screen pt-32 pb-20">
        <div className="eggi-container text-center">
          ...
          <h1 className="display-title text-4xl mb-3">{t("cart_empty_title")}</h1>
          <p className="text-[#7a7266] font-medium mb-6">{t("cart_empty_text")}</p>
  ...
  return (
    <div className="eggi-cream min-h-screen pt-28 md:pt-32 pb-20">
      <div className="eggi-container">
        <h1 className="display-title text-5xl mb-8">{t("cart_title")}</h1>
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white hard-border rounded-2xl p-6 h-fit" style={{ boxShadow: "6px 6px 0 0 #1C1A17" }}>
            <h2 className="font-extrabold text-xl mb-4">{t("order_summary")}</h2>
            <div className="space-y-2 text-sm font-semibold">
              <div className="flex justify-between"><span className="text-[#7a7266]">{t("subtotal")}</span><span>{SITE.currency}{subtotal.toFixed(2)}</span></div>
              <div className="flex justify-between"><span className="text-[#7a7266]">{t("shipping")}</span><span className="text-[#2f8f5b]">{t("free")}</span></div>
            </div>
            ...
            <button onClick={() => navigate("/checkout")} className="btn-brutal w-full mt-6 eggi-dark text-white font-bold px-6 py-3.5 rounded-xl hard-border" style={{ boxShadow: "3px 3px 0 0 #FFD84D" }}>{t("checkout")}</button>
            <Link to="/shop" className="block text-center mt-3 text-sm font-bold underline">{t("continue_shopping")}</Link>
          </div>
        </div>
      </div>
    </div>

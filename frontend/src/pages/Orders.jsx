...
const StatusBadge = ({ status }) => {
  ...
  return <span className="text-xs font-extrabold px-3 py-1 rounded-lg hard-border" style={{ background: m.bg, color: m.color }}>{m.label}</span>;
};
      ...
      {ORDER_STATUSES.map((s, i) => (
        <React.Fragment key={s.key}>
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] font-bold mt-1 text-center w-14">{s.label}</span>
          </div>
...
const Orders = () => {
  ...
  if (loading || busy) return <div className="pt-40 text-center font-bold">Loading orders...</div>;
    ...
    return (
      <div className="eggi-cream min-h-screen pt-32 pb-20">
        <div className="eggi-container text-center">
          ...
          <h1 className="display-title text-4xl mb-3">No orders yet.</h1>
  ...
  return (
    <div className="eggi-cream min-h-screen pt-28 md:pt-32 pb-20">
      <div className="eggi-container max-w-4xl">
        <h1 className="display-title text-5xl mb-8">My orders</h1>
          ...
          {orders.map((o) => (
            <div key={o.id} className={`bg-white hard-border rounded-2xl overflow-hidden ${highlight === o.id ? "ring-2 ring-[#FFD84D]" : ""}`} style={{ boxShadow: "4px 4px 0 0 #1C1A17" }}>
              <button onClick={() => setOpen(open === o.id ? null : o.id)} className="w-full flex items-center justify-between gap-4 p-5 text-left">
                <div>
                  <p className="font-extrabold">Order #{shortId(o.id)}</p>
                  <p className="text-sm text-[#7a7266] font-semibold">{new Date(o.createdAt).toLocaleDateString()} \u00b7 {o.items?.length} item(s) \u00b7 {SITE.currency}{o.total?.toFixed(2)}</p>
                </div>
                <div className="flex items-center gap-3"><StatusBadge status={o.status} /><ChevronDown size={18} className={`transition-transform ${open === o.id ? "rotate-180" : ""}`} /></div>
              </button>
              {open === o.id && (
                <div className="px-5 pb-5 border-t-2 border-[#1C1A17] pt-4">
                  {o.status !== "cancelled" && <Tracker status={o.status} />}
                  ...
                  {o.shippingInfo && (
                    <div className="mt-4 text-sm bg-[#FCF7E8] hard-border rounded-xl p-4">
                      <p className="font-extrabold mb-1">Shipping to</p>
